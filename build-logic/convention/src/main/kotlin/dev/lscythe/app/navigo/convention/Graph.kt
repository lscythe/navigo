...
private class Graph(
    ...
    fun dependencies(): Map<String, Set<Pair<String, String>>> =
        dependencies
            .mapKeys { it.key.path }
...
private abstract class GraphDumpTask : DefaultTask() {
    ...
    @get:Input abstract val dependencies: MapProperty<String, Set<Pair<String, String>>>

    @get:Input abstract val plugins: MapProperty<String, PluginType>
    ...
    private fun mermaid() = buildString {
        val dependencies: Set<Dependency> =
            dependencies.get().flatMapTo(mutableSetOf()) { (project, entries) ->
        ...
        appendLine("graph TB")
        // Nodes and subgraphs
        val (rootProjects, nestedProjects) =
            dependencies
                .map { listOf(it.project, it.dependency) }
                .flatten()
                .toSet()
                .plus(
                    projectPath.get()
        ...
        orderedGroups.forEach { (outerGroup, innerGroups) ->
            ...
            val inboundCounts = innerGroups.associate { (group, _) ->
                group to
                    dependencies.count { dep ->
                        dep.dependency.substringBeforeLast(":") == group &&
                            dep.project.substringBeforeLast(":") != group
                    }
            }
            innerGroups
                .sortedWith(
                    compareBy(
                        { (group, _) -> inboundCounts.getValue(group) },
                        { -it.value.size },
                    )
                )
...
private abstract class GraphUpdateTask : DefaultTask() {
    ...
    @get:InputFile @get:PathSensitive(NONE) abstract val input: RegularFileProperty

    @get:InputFile @get:PathSensitive(NONE) abstract val legend: RegularFileProperty
    ...
    private fun String.trimTrailingNewLines() =
        lines().dropLastWhile(String::isBlank).joinToString(System.lineSeparator())
}
